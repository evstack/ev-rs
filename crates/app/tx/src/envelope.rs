...
pub mod tx_type {
    ...
    // Custom Evolve transaction types (0x80-0xFF range)
    /// Batch transaction with multiple messages.
    pub const EVOLVE_BATCH: u8 = 0x80;
    /// Sponsored (gasless) transaction.
    pub const EVOLVE_SPONSORED: u8 = 0x81;
    /// Scheduled/delayed execution transaction.
    pub const EVOLVE_SCHEDULED: u8 = 0x82;
}

/// Unified transaction envelope that can hold any supported transaction type.
///
/// This implements EIP-2718 typed transaction envelopes, allowing the system
/// to support both standard Ethereum transactions and custom Evolve types.
#[derive(Clone, Debug)]
pub enum TxEnvelope {
    ...
    // Future: Add custom Evolve types here
    // EvolveBatch(SignedBatchTx),
    // EvolveSponsored(SignedSponsoredTx),
}
